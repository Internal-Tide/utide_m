...
def ut_E_m(t, tref, frq, lind, lat, ngflgs, prefilt, fuv_cache=None):
    ...
    if ngflgs[1] and ngflgs[3]:
        ...
        if fuv_cache is None:
            F, U, V = FUV(t, tref, lind, lat, ngflgs)
        else:
            if len(fuv_cache) == 3:
                F, U, V = fuv_cache
            elif len(fuv_cache) == 4:
                F_cache, U_cache, V_cache, lind_cache = fuv_cache
                lind_cache = np.atleast_1d(lind_cache)

                if V_cache.shape[0] != nt:
                    raise ValueError(
                        "fuv_cache has incompatible time dimension: "
                        f"expected nt={nt}, got V={V_cache.shape}",
                    )

                if F_cache.shape[0] not in (1, nt) or U_cache.shape[0] not in (1, nt):
                    raise ValueError(
                        "fuv_cache has incompatible time dimension: "
                        f"expected F/U first dim in (1, {nt}), got F={F_cache.shape}, U={U_cache.shape}",
                    )

                cache_pos = {int(l): i for i, l in enumerate(lind_cache.tolist())}
                pos = []
                missing = []
                for l in lind.tolist():
                    key = int(l)
                    if key in cache_pos:
                        pos.append(cache_pos[key])
                    else:
                        missing.append(key)

                if missing:
                    F_miss, U_miss, V_miss = FUV(t, tref, np.array(missing, dtype=int), lat, ngflgs)

                    F_parts = []
                    U_parts = []
                    V_parts = []
                    miss_index = {m: i for i, m in enumerate(missing)}

                    for l in lind.tolist():
                        key = int(l)
                        if key in cache_pos:
                            i = cache_pos[key]
                            F_parts.append(F_cache[:, i : i + 1])
                            U_parts.append(U_cache[:, i : i + 1])
                            V_parts.append(V_cache[:, i : i + 1])
                        else:
                            j = miss_index[key]
                            F_parts.append(F_miss[:, j : j + 1])
                            U_parts.append(U_miss[:, j : j + 1])
                            V_parts.append(V_miss[:, j : j + 1])
